...
function authentication ($http, $window,jwtHelper) {
  ...
  var saveToken = function (token) {
    if ( token ) {
        ...
        $window.localStorage.setItem('profesor', token);
    }
  };

  var getToken = function () {
    return $window.localStorage.getItem('profesor');
  };

  var login = function(ayudante, cb) {
    return $http.post('/api/v1/profesores/login', ayudante).then(function(data) {
  ...
  var logout = function() {
    $window.localStorage.removeItem('profesor');
  };
