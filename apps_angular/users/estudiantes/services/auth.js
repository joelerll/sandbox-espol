angular.module('estudiantesApp').service('auth', authentication);
...
function authentication ($http, $window,jwtHelper) {
  ...
  var saveToken = function (token) {
    if ( token ) {
        ...
        $window.localStorage.setItem('estudiantes', token);
    }
  };

  var getToken = function () {
    return $window.localStorage.getItem('estudiantes');
  };

  var login = function(ayudante, cb) {
    return $http.post('/api/v1/estudiantes/login', ayudante).then(function(data) {
